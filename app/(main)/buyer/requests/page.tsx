  ...
  return (
    <div className="py-8 page-container">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">My Grail Requests</h1>
          <p className="text-neutral-500 text-sm mt-1">Manage all your grail requests in one place</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Pending Offers", value: stats.offers, color: "text-orange-500" },
          { label: "Completed", value: stats.completed, color: "text-purple-700" },
        ].map((s) => (
          <div key={s.label} className="bg-neutral-900 rounded-xl border border-neutral-800 p-4 text-center">
      ...
      {grails.length === 0 ? (
        <div className="bg-neutral-900 rounded-xl border border-dashed border-neutral-800 p-16 text-center">
          ...
          <h3 className="font-medium text-neutral-400 mb-1">No requests yet</h3>
          <p className="text-sm text-neutral-500 mb-5">Post your first grail request and let sellers find you</p>
          <Link href="/buyer/create" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500 text-white font-semibold text-sm hover:bg-orange-400 transition-colors">
        ...
        <div className="bg-neutral-900 rounded-xl border border-neutral-800 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-neutral-800 border-b border-neutral-800">
              <tr>
                <th className="text-left px-4 py-3 font-semibold text-neutral-400">Request</th>
                <th className="text-left px-4 py-3 font-semibold text-neutral-400 hidden md:table-cell">Category</th>
                <th className="text-left px-4 py-3 font-semibold text-neutral-400 hidden sm:table-cell">Budget</th>
                <th className="text-center px-4 py-3 font-semibold text-neutral-400">Offers</th>
                <th className="text-left px-4 py-3 font-semibold text-neutral-400 hidden lg:table-cell">Posted</th>
            ...
            <tbody className="divide-y divide-neutral-800">
              {grails.map((grail) => {
                ...
                return (
                  <tr key={grail.id} className="hover:bg-neutral-800 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-neutral-800 overflow-hidden flex-shrink-0">
                          {images[0]
                            ? <img src={images[0]} alt="" className="w-full h-full object-cover" />
                            : <Trophy className="w-5 h-5 text-neutral-700 m-auto mt-2.5" />}
                        </div>
                        <div>
                          <Link href={`/grails/${grail.id}`} className="font-medium text-white hover:text-orange-500 line-clamp-1 max-w-[200px]">
                            {grail.title}
                          </Link>
                          <div className="text-xs text-neutral-500 mt-0.5 flex items-center gap-1">
                            ...
                            <span className="text-orange-500 font-semibold">Score {score}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-neutral-400 hidden md:table-cell">
                      <span className="text-xs px-2 py-0.5 rounded-full bg-neutral-800">{getCategoryLabel(grail.category)}</span>
                    </td>
                    <td className="px-4 py-3 font-medium text-white hidden sm:table-cell">
                      {formatCurrency(grail.budgetMin)}–{formatCurrency(grail.budgetMax)}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {pending > 0 ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-500/15 text-orange-300 text-xs font-semibold">
                          <Package className="w-3 h-3" /> {pending} new
                        </span>
                      ) : (
                        <span className="text-neutral-500 text-xs">{grail._count.offers} total</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-neutral-500 text-xs hidden lg:table-cell">{formatDate(grail.createdAt)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/grails/${grail.id}`} className="text-xs font-medium text-orange-500 hover:text-orange-400">
                        View →
                      </Link>
                    </td>
                  </tr>
