    ...
    <div className="py-8 page-container max-w-4xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Bookmark className="w-6 h-6 text-orange-500" fill="currentColor" /> Saved Watchlist
        </h1>
      ...
      {saved.length === 0 ? (
        <div className="bg-neutral-900 rounded-xl border border-dashed border-neutral-800 p-16 text-center">
          ...
          <h3 className="font-medium text-neutral-400 mb-1">No saved grails</h3>
          <p className="text-sm text-neutral-500 mb-5">Save grails from the browse page to track them here</p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {saved.map(({ id, grailRequest: g, createdAt }) => {
            ...
            return (
              <div key={id} className="bg-neutral-900 rounded-xl border border-neutral-800 overflow-hidden hover:shadow-sm transition-all group">
                ...
                <div className="p-4">
                  <Link href={`/grails/${g.id}`} className="font-semibold text-white hover:text-orange-500 text-sm line-clamp-2">
                    {g.title}
                  </Link>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-neutral-500">{getCategoryLabel(g.category)}</span>
                    <span className="text-sm font-semibold text-orange-500">{formatCurrency(g.budgetMin)}–{formatCurrency(g.budgetMax)}</span>
                  </div>
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-neutral-800">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-neutral-500 hover:text-red-500 hover:bg-red-50 transition-colors"
                    >
