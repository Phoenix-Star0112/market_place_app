...
export default async function CheckoutPage({ params }: PageProps) {
  ...
  return (
    <div className="py-8 page-container max-w-2xl">
      <Link href={`/grails/${offer.grailRequestId}`} className="inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-white mb-6">
        <ArrowLeft className="w-4 h-4" /> Back
      </Link>

      <h1 className="text-2xl font-bold text-white mb-2">Complete Your Purchase</h1>
      <p className="text-neutral-500 text-sm mb-6">Review the offer details and proceed to secure payment</p>
      ...
      <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-6 mb-4">
        ...
        <div className="flex items-start gap-4 pb-5 border-b border-neutral-800 mb-5">
          <div className="w-12 h-12 rounded-xl bg-neutral-800 flex items-center justify-center flex-shrink-0">
            <Package className="w-5 h-5 text-neutral-500" />
          </div>
          <div className="flex-1">
            <p className="font-semibold text-white">{offer.grailRequest.title}</p>
            <p className="text-sm text-neutral-500 mt-0.5">Offered by {offer.seller.name}</p>
            <p className="text-xs text-neutral-500 mt-1 line-clamp-2">{offer.description}</p>
          </div>
        </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            <span className="text-neutral-400">Buyer protection fee</span>
            <span className="text-neutral-500 text-xs mt-0.5">Included</span>
          </div>
          <div className="flex justify-between pt-3 border-t border-neutral-800">
            <span className="font-semibold text-white">Total</span>
            <span className="font-bold text-xl text-white">{formatCurrency(offer.price)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <button
          className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-orange-500 text-white font-bold text-base hover:bg-orange-400 transition-colors"
