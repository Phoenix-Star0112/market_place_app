    ...
    <div className="py-8 page-container max-w-3xl">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-dashed border-slate-200 p-16 text-center">
          ...
          <h3 className="font-medium text-slate-600 mb-1">No conversations yet</h3>
          <p className="text-sm text-slate-400 mb-5">Messages with sellers/buyers will appear here</p>
          ...
          {conversations.map((conv) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-4 bg-white rounded-xl border border-slate-200 p-4 hover:shadow-sm hover:border-slate-300 transition-all"
              >
                <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center font-bold text-yellow-800 flex-shrink-0">
                  {otherUser.name?.[0]?.toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-slate-900 text-sm">{otherUser.name}</span>
                  ...
                  {lastMessage && (
                    <p className="text-xs text-slate-400 truncate mt-0.5">{lastMessage.content}</p>
                  )}
                </div>
                {unread > 0 && (
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-yellow-400 text-yellow-900 text-[10px] font-bold flex items-center justify-center">
                    {unread}
                  </span>
                )}
              </Link>
