  ...
  return (
    <div className="py-10 page-container max-w-3xl">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Set Up Your Seller Account</h1>
        <p className="text-slate-500">Connect Stripe to receive fast, secure payouts on every sale</p>
      </div>
        ...
        {STEPS.map((s) => (
          <div key={s.id} className="flex flex-col items-center z-10 flex-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
              ...
              step === s.id ? "bg-yellow-400 border-yellow-400 text-yellow-900" :
              "bg-white border-slate-200 text-slate-400"
            }`}>
              {step > s.id ? <CheckCircle className="w-4 h-4" /> : s.id}
            </div>
            <p className="text-[10px] text-slate-500 mt-1.5 text-center font-medium max-w-[60px] leading-tight">{s.title}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {step === 1 && (
          <div className="p-8">
            <h2 className="text-xl font-bold text-slate-900 mb-2">Connect with Stripe</h2>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-8">
              {[
                ...
                <div key={f.label} className="p-4 rounded-xl bg-slate-50 text-center">
            ...
            <p className="text-center text-xs text-slate-400 mt-3">
              ...
              <a href="https://stripe.com/connect-account/legal" target="_blank" rel="noopener noreferrer" className="underline hover:text-slate-600">
        ...
        {step === 2 && (
          <div className="p-8 text-center">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2">Stripe Connected!</h2>
            <p className="text-slate-500 text-sm mb-6">
            ...
            <div className="flex gap-3 justify-center">
              <Link href="/grails" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-yellow-400 text-slate-900 font-semibold hover:bg-yellow-300 transition-colors">
                Browse Grails <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/seller/dashboard" className="inline-flex items-center px-5 py-2.5 rounded-lg border border-slate-200 text-slate-700 font-semibold hover:bg-slate-50 transition-colors">
                Seller Dashboard
              </Link>
            </div>
          </div>
      ...
      <div className="mt-6 bg-slate-50 rounded-xl border border-slate-200 p-4 text-sm text-slate-600">
        <strong>Dev note:</strong> Add your <code className="bg-slate-200 px-1 rounded text-xs">STRIPE_SECRET_KEY</code> and{" "}
        <code className="bg-slate-200 px-1 rounded text-xs">STRIPE_PUBLISHABLE_KEY</code> to{" "}
        <code className="bg-slate-200 px-1 rounded text-xs">.env.local</code> to enable real Stripe Connect.
      </div>
    </div>
  )
}
