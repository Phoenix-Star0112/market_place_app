...
export default function GrailDetailPage() {
  ...
  if (loading) return <div className="py-16 page-container flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-yellow-500" /></div>
  if (!grail) return (
    <div className="py-16 page-container text-center">
      <Trophy className="w-12 h-12 text-slate-300 mx-auto mb-3" />
      <h2 className="font-semibold text-slate-700">Grail request not found</h2>
      <Link href="/grails" className="text-yellow-600 text-sm mt-2 inline-block hover:text-yellow-700">← Browse all grails</Link>
    </div>
  )
  ...
  return (
    <div className="py-8 page-container">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/grails" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900">
          <ArrowLeft className="w-4 h-4" /> Browse
        </Link>
        <span className="text-slate-300">/</span>
        <span className="text-sm text-slate-900 font-medium truncate max-w-xs">{grail.title}</span>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div className="p-6">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-4">
                <span className="text-xs px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 font-medium">{getCategoryLabel(grail.category)}</span>
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${grail.status === "ACTIVE" ? "bg-green-100 text-green-800" : grail.status === "IN_PROGRESS" ? "bg-blue-100 text-blue-800" : grail.status === "COMPLETED" ? "bg-purple-100 text-purple-800" : "bg-slate-100 text-slate-700"}`}>
                  {grail.status.replace("_", " ").charAt(0) + grail.status.replace("_", " ").slice(1).toLowerCase()}
                </span>
                <GrailScoreBadge score={score} size="sm" showLabel />
              </div>

              <h1 className="text-2xl font-bold text-slate-900 mb-2">{grail.title}</h1>
              <p className="text-slate-600 leading-relaxed text-sm mb-5">{grail.description}</p>

              {/* Image gallery */}
              <ImageGallery images={images} title={grail.title} thumbnailClass="h-56" />

              {/* Meta row */}
              <div className="flex flex-wrap gap-5 mt-5 pt-5 border-t border-slate-100 text-sm text-slate-500">
                <span className="flex items-center gap-1.5"><DollarSign className="w-4 h-4" /> Budget: <strong className="text-slate-900">{formatCurrency(grail.budgetMin)}–{formatCurrency(grail.budgetMax)}</strong></span>
                <span className="flex items-center gap-1.5"><Star className="w-4 h-4" /> Condition: <strong className="text-slate-900">{getConditionLabel(grail.condition)}</strong></span>
              ...
              <div className="flex flex-wrap gap-2 mt-5">
                <button onClick={toggleSave} disabled={savingWatch} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium transition-all ${isSaved ? "bg-yellow-50 border-yellow-300 text-yellow-700" : "border-slate-200 text-slate-600 hover:border-yellow-300 hover:bg-yellow-50"}`}>
                  {savingWatch ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Bookmark className="w-3.5 h-3.5" fill={isSaved ? "currentColor" : "none"} />}
                  {isSaved ? "Saved" : "Save"}
                </button>
                <button onClick={handleShare} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-all">
                  <Share2 className="w-3.5 h-3.5" /> Share
                </button>
                <Link href={`/buyer/create?from=${grail.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-all">
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                <h2 className="text-lg font-bold text-slate-900">Offers <span className="text-slate-400 font-normal text-sm">({grail._count.offers})</span></h2>
                {pendingOffers.length >= 2 && (
                  <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs font-medium">
                    <button onClick={() => setActiveTab("all")} className={`px-3 py-1.5 transition-colors ${activeTab === "all" ? "bg-yellow-400 text-yellow-900" : "text-slate-600 hover:bg-slate-50"}`}>All</button>
                    <button onClick={() => setActiveTab("compare")} className={`px-3 py-1.5 transition-colors ${activeTab === "compare" ? "bg-yellow-400 text-yellow-900" : "text-slate-600 hover:bg-slate-50"}`}>Compare</button>
                  </div>
                )}
              </div>
              {isSeller && grail.status === "ACTIVE" && !showOfferForm && (
                <button onClick={() => setShowOfferForm(true)} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-yellow-400 text-slate-900 font-semibold text-sm hover:bg-yellow-300 transition-colors">
            ...
            {/* Offer form */}
            {showOfferForm && (
              <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-5 mb-4">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-slate-900">Submit Your Offer</h3>
                  <button onClick={() => setShowOfferForm(false)} className="text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
                </div>
                {marketData && <div className="mb-4"><MarketPulse data={marketData} compact /></div>}
                <form onSubmit={submitOffer} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Your Price (USD)</label>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Buyer&apos;s budget: {formatCurrency(grail.budgetMin)}–{formatCurrency(grail.budgetMax)}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Item Condition</label>
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setShowOfferForm(false)} className="flex-1 px-4 py-2 rounded-lg border border-slate-200 text-slate-700 font-medium text-sm hover:bg-slate-50 transition-colors">Cancel</button>
                    <button type="submit" disabled={submittingOffer} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-yellow-400 text-slate-900 font-semibold text-sm hover:bg-yellow-300 disabled:opacity-50 transition-colors">
            ...
            {activeTab === "compare" && pendingOffers.length >= 2 ? (
              ...
              grail.offers.length === 0 ? (
                <div className="bg-white rounded-xl border border-dashed border-slate-200 p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500">No offers yet{isSeller ? " — be the first!" : ""}</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {grail.offers.map((offer) => {
                    ...
                    return (
                      <div key={offer.id} className={`bg-white rounded-xl border-2 overflow-hidden ${offer.status === "ACCEPTED" ? "border-green-300" : "border-slate-200"}`}>
                        <div className="p-5">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full overflow-hidden bg-yellow-100 flex items-center justify-center font-bold text-yellow-800 flex-shrink-0">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <Link href={`/profile/${offer.seller.id}`} className="font-semibold text-slate-900 text-sm hover:text-yellow-600">{offer.seller.name}</Link>
                            ...
                            <div className="text-right flex-shrink-0">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${offer.status === "PENDING" ? "bg-yellow-100 text-yellow-800" : offer.status === "ACCEPTED" ? "bg-green-100 text-green-800" : offer.status === "DECLINED" ? "bg-red-100 text-red-800" : "bg-slate-100 text-slate-600"}`}>
                          ...
                          <p className="text-sm text-slate-600 mt-3 leading-relaxed">{offer.description}</p>
                          <div className="mt-2 text-xs text-slate-500">Condition: <span className="font-medium text-slate-700">{getConditionLabel(offer.condition)}</span></div>
                          ...
                          {isOwner && offer.status === "PENDING" && grail.status === "ACTIVE" && (
                            <div className="flex gap-2 mt-4">
                              ...
                              <Link href={`/messages/${offer.id}`} className="px-3 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
                          ...
                          {offer.status === "ACCEPTED" && (
                            <div className="mt-4 flex gap-2">
                              ...
                              <Link href={`/messages/${offer.id}`} className="px-3 py-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">
                                <MessageSquare className="w-4 h-4" />
                              </Link>
                            </div>
                          )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            <h3 className="font-semibold text-slate-900 mb-4 text-sm">Posted by</h3>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full overflow-hidden bg-yellow-100 flex items-center justify-center font-bold text-yellow-800 text-lg">
                {grail.user.image ? <img src={grail.user.image} alt="" className="w-full h-full object-cover" /> : grail.user.name?.[0]?.toUpperCase()}
              </div>
              <div>
                <Link href={`/profile/${grail.user.id}`} className="font-semibold text-slate-900 hover:text-yellow-600">{grail.user.name}</Link>
                ...
                <div className="text-xs text-slate-500 mt-0.5">{grail.user._count.grailRequests} requests · Joined {formatDate(grail.user.createdAt)}</div>
              </div>
            </div>
          </div>
          ...
          {/* Details */}
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            <h3 className="font-semibold text-slate-900 mb-4 text-sm">Request Details</h3>
          ...
          {/* Sell CTA */}
          {isSeller && grail.status === "ACTIVE" && !showOfferForm && (
            <button onClick={() => setShowOfferForm(true)} className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-yellow-400 text-slate-900 font-bold hover:bg-yellow-300 transition-colors">
